...
void hal_i2c_test_task(void *pvParameter)
{
    ...
    while(1)
    {
        //for(i=0;i<0xff;i++])
        {
            ...
            {
                log_printf(HALI2C_TAG, LOG_DEBUG, "成功 地址:0x%02x 数据:0x%02x \r\n",i,dummy);
            }else
            {
                log_printf(HALI2C_TAG, LOG_DEBUG, "失败 地址:0x%02x 数据:0x%02x \r\n",i,dummy);
            }
...
void i2c2_init(void)
{
    ...
    {
        log_printf(HALI2C_TAG, LOG_DEBUG, "i2c2初始化成功\r\n");
        //xTaskCreate(hal_i2c_test_task,"hal_i2c_test_task",1024*5,NULL,12,NULL);
    }else
    {
        log_printf(HALI2C_TAG, LOG_DEBUG, "i2c2初始化失败\r\n");
    }
...
void i2c2_uninit(void)
{
    ...
    log_printf(HALI2C_TAG, LOG_DEBUG, "I2C de-initialized successfully");
}
