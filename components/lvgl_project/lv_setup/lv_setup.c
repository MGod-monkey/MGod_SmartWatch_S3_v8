...
void setupView_init(void)
{
    lv_disp_t *disp = lv_disp_get_default();
    lv_obj_t* bg = lv_obj_create(lv_scr_act());
    lv_obj_remove_style_all(bg);
    lv_obj_set_size(bg, lv_disp_get_hor_res(disp), lv_disp_get_ver_res(disp));
    ...
    lv_label_set_text(label, "S-WATCH");
    lv_obj_center(label);
    ...
    lv_timer_t * timer = lv_timer_create(setupView_delete, 1000, NULL);
