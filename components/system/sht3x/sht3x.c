...
sht3x_sensor_t* sht3x_init_sensor(uint8_t bus, uint8_t addr)
{
    ...
    if (!sht3x_reset(dev))
    {
        log_printf(SHT3x_TAG, LOG_ERROR, "could not reset the sensor");
    }
    
    // check again the status after clear status command
    if (!sht3x_get_status(dev, &status))
    {
        log_printf(SHT3x_TAG, LOG_ERROR, "could not get sensor status");
        free(dev);
        return NULL;       
    }
    log_printf(SHT3x_TAG, LOG_DEBUG, "sensor initialized");
    return dev;
}
...
bool sht3x_start_measurement (sht3x_sensor_t* dev, sht3x_mode_t mode, sht3x_repeat_t repeat)
{
    ...
    {
        log_printf(SHT3x_TAG, LOG_ERROR, "could not send start measurment command");
...
bool sht3x_get_raw_data(sht3x_sensor_t* dev, sht3x_raw_data_t raw_data)
{
    ...
    if (!dev->meas_started)
    {
        log_printf(SHT3x_TAG, LOG_DEBUG, "measurement is not started");
    ...
    {
        log_printf(SHT3x_TAG, LOG_ERROR, "measurement is still running");
    ...
    {
        log_printf(SHT3x_TAG, LOG_ERROR, "send fetch command failed");
    ...
    {
        log_printf(SHT3x_TAG, LOG_ERROR, "read raw data failed");
    ...
    {
        log_printf(SHT3x_TAG, LOG_ERROR, "CRC check for temperature data failed");
    ...
    {
        log_printf(SHT3x_TAG, LOG_ERROR, "CRC check for humidity data failed");
...
static bool sht3x_send_command(sht3x_sensor_t* dev, uint16_t cmd)
{
    ...
    int err = hal_i2c_master_write_byte_len(dev->addr, 0, data, 2);

    if (err)
    {
        dev->error_code |= (err == -EBUSY) ? SHT3x_I2C_BUSY : SHT3x_I2C_SEND_CMD_FAILED;
        log_printf(SHT3x_TAG, LOG_ERROR, "i2c error %d on write command %02x", err, cmd);
        return false;
    }
...
{
    if (!dev) return false;
    int err = hal_i2c_master_read_byte_len(dev->addr, 0, data, len);
        
    if (err)
    {
        dev->error_code |= (err == -EBUSY) ? SHT3x_I2C_BUSY : SHT3x_I2C_READ_FAILED;
        log_printf(SHT3x_TAG, LOG_ERROR, "error %d on read %d byte", err, len);
        return false;
    }
...
static bool sht3x_reset (sht3x_sensor_t* dev)
{
    ...
    log_printf(SHT3x_TAG, LOG_DEBUG, "soft-reset triggered");
